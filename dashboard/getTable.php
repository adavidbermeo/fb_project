...
use metrics\posts\PostInsights;
    ...
    if(isset($_REQUEST['selected'])){
        ...
        getData($ad_account_id, $id_page, $_REQUEST['start_time']);
    }

function getData($ad_account_id, $id_page, $start_time, $end_time = 0){

    //Page Insights
    $PageInsights = new PageInsights($id_page,$ad_account_id,$start_time);
    $page_insights_array = $PageInsights->account_info_array;
    

    //Ad Insights
    $AdInsights = new AdInsights($ad_account_id,$start_time);
    $ad_insights_array = $AdInsights->adInsights;

    // // Post Insights
    $PostInsights = new PostInsights($id_page,$ad_account_id, $start_time);
    $post_insights_array = $PostInsights->adPerformance;

    //Adset Insights   
    $AdSetInsights = new AdsetInsights($ad_account_id,$start_time);  
